...
# Multi-target translation in a single call
...
- include: [src/mdpo_llm/processor.py, src/mdpo_llm/prompts.py, src/mdpo_llm/batch.py, src/mdpo_llm/__main__.py, src/mdpo_llm/__init__.py, tests/test_batch.py, tests/test_batched_processing.py, README.md, CHANGELOG.md]
- exclude: []
...
- Running actual multi-language benchmarks against a live LLM — structure the code so a human can run the "canonical-seeded" comparison after merge, but do not require live API calls during the task.
